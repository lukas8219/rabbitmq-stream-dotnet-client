...
.Creating an environment with disposable pattern
[source,c#,indent=0]
--------
include::{test-examples}/StreamSystemUsage.cs[tag=create-disposable]
--------

`StreamSystem` implements `IAsyncDisposable`, so you can use the `using` statement to automatically close it after usage.
...
|`AuthMechanism`
|Authentication mechanism to use (e.g. `Plain` or `External` for TLS client certificates). See <<create-tls-external-auth>> for external authentication.
|`Plain`
...
|`RpcTimeOut`
|Timeout for RPC calls (e.g. PeerProperties, QueryMetadata). Must be at least 1 second. Low values can cause false errors.
|`10 seconds`

|`LookupLocatorStrategy`
|Strategy to reconnect the locator client in case of connection failure. See `ILookupLocatorStrategy`.
|`BackOffLookupLocatorStrategy`
...
|`SocketOptions`
|The <<socket-options, socket options>> to use for the connections.
|`null` (default socket options)
...
The client uses the standard .NET logging abstraction `Microsoft.Extensions.Logging.ILogger` from the `Microsoft.Extensions.Logging.Abstractions` package.
Logging is optional: when no logger is provided, the client uses a no-op logger and does not emit any log output.

The following factory methods accept an optional `ILogger` (or `ILogger<T>`) parameter:

* `StreamSystem.Create(StreamSystemConfig config, ILogger<StreamSystem> logger = null)` – logger for the stream system
* `Producer.Create(ProducerConfig producerConfig, ILogger<Producer> logger = null)` – reliable producer
* `Consumer.Create(ConsumerConfig consumerConfig, ILogger<Consumer> logger = null)` – reliable consumer
* `StreamSystem.CreateRawProducer(RawProducerConfig rawProducerConfig, ILogger logger = null)` – raw producer
* `StreamSystem.CreateRawConsumer(RawConsumerConfig rawConsumerConfig, ILogger logger = null)` – raw consumer
* `StreamSystem.CreateRawSuperStreamProducer(RawSuperStreamProducerConfig rawSuperStreamProducerConfig, ILogger logger = null)` – super stream producer
* `StreamSystem.CreateSuperStreamConsumer(RawSuperStreamConsumerConfig rawSuperStreamConsumerConfig, ILogger logger = null)` – super stream consumer

A concrete logging implementation (for example `Microsoft.Extensions.Logging.Console`) is not shipped with the client; you add the logging packages you need to your application.

Example: configuring a logger with `LoggerFactory` and passing it when creating the stream system and producers/consumers:

.Configuring ILogger with LoggerFactory
[source,c#,indent=0]
--------
include::{test-examples}/GettingStarted.cs[tag=sample-logging]
--------

<1> Create a logger for the reliable producer
<2> Create a logger for the reliable consumer
<3> Create a logger for the stream system

Use the same logger (or category) when creating the `StreamSystem` and the producers/consumers so that connection and protocol events are visible in your logs.
You can filter by the `RabbitMQ.Stream` namespace to control verbosity.

[[socket-options]]
===== Socket Options
The `SocketOptions` property allows configuring the socket options for the connections to the broker.
Default is `null` with the default values:
```csharp
 socket.NoDelay = true;
...
Starting from version TODO (https://github.com/rabbitmq/rabbitmq-stream-dotnet-client/pull/466[#466]), the client ships a built-in `DnsAddressResolver` that relies on DNS round-robin instead of a dedicated load balancer.
...
|`Filter`
|Function used to set the value of the filter. The filter enables the server to filter the messages sent to the consumer.
|`null`
...
|The strategy to check if the stream is available (e.g. during broker restart). From `ReliableConfig`.
|`null`

|`Identifier`
|Identifies the entity in logs. From `ReliableConfig`. No effect on the server.
|`null`
...
Avoid setting the `Reference` property in the `ProducerConfig`; it enables deduplication and you could have unexpected results.
...
|Callback invoked when the consumer becomes active (single active consumer only). Lets the consumer decide where to start consuming from. See <<consumer-update-listener,Reacting to Consumer State Change>>.
|`null`

|`InitialCredits`
|Initial number of chunks the consumer will receive before requesting more. Higher values can increase throughput but use more memory.
|`2`

|`Filter`
|Consumer filter to receive only messages matching the filter (bloom filter with optional post-filter).
|`null`

|`FlowControl`
|Strategy for when the broker sends more messages. See <<consumer-flow-control,Flow Control>>.
|`new FlowControl()` (default strategy)
...
|`Identifier`
|Identifies the entity in logs and UI. From `ReliableConfig`. No effect on the server.
|`null`
